...
import fpget
from distiller_zoo.aploss import aploss
...
def train_mixdistill(epoch, train_loader, module_list, criterion_list, optimizer, alpha, patch_size, opt):
    ...
    print("ready!!!!!!!!")
    for name,m in model_t._modules.items():
       print(name) 
    for name,m in model_s._modules.items():
       print(name) 
    
    # print(model_s._modules.items())
    print("yes!!!!!!!!")
    ...
    for idx, data in enumerate(train_loader):
        ...
        Thread=0.5
        alpha=0.5
        index_s=fpget.feature_index_get(model_s,"block3")
        index_t=fpget.feature_index_get(model_t,"block3")
        logit_s=model_s(input)
        loss_fn = torch.nn.MSELoss(reduction='none')
        with torch.no_grad():
            logit_t=model_t(input)
        fmap_s=fpget.featuremap_get(index_s)
        fmap_t=fpget.featuremap_get(index_t)
        loss_ap=aploss(fmap_t,fmap_s,loss_fn,Thread)
        loss_cls = criterion_cls(logit_s, target)
        loss=alpha*loss_ap+(1-alpha)*loss_cls
        # ===================forward_new_end=====================
